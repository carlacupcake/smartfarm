...
class Member:
    ...
    def __init__(
        ...
        if self.values is None or (isinstance(values, np.ndarray) and values.size == 0):
            self.values = np.zeros(shape=(bounds.upper_bounds.shape[0], 1))
    ...
    def get_cost(self) -> float:
        ...
        h = np.full(total_time_steps, self.initial_conditions.h0)
        A = np.full(total_time_steps, self.initial_conditions.A0)
        N = np.full(total_time_steps, self.initial_conditions.N0)
        c = np.full(total_time_steps, self.initial_conditions.c0)
        P = np.full(total_time_steps, self.initial_conditions.P0)
        ...
        for t in range(total_time_steps - 1):
            S = precipitation[t]; T = temperature[t]; R = radiation[t]
            W = irrigation[t]; F = fertilizer[t]
