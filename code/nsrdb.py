...
def data_download(API_KEY, EMAIL, BASE_URL, POINTS):
    input_data = {
        'attributes': 'air_temperature,dew_point,dhi,surface_albedo,dni,ghi',
        'interval': '60',
        'to_utc': 'false',
    ...
    for name in ['tmy','tmy-2021','tdy-2021','tgy-2021','tmy-2022','tdy-2022','tgy-2022']:
        print(f"Processing name: {name}")
        for id, location_ids in enumerate(POINTS):
            input_data['names'] = [name]
            input_data['location_ids'] = location_ids
            print(f'Making request for POINTS group {id + 1} of {len(POINTS)}...')

            if '.csv' in BASE_URL:
                url = BASE_URL + urllib.parse.urlencode(data, True)
                # Note: CSV format is only supported for single POINTS requests
                ...
                data.to_csv(f'nsdrb_{POINTS}.csv')
            else:
                headers = {
                  'x-api-key': API_KEY
                }
